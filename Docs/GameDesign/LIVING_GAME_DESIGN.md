...
- A resource hex exposes exactly three worker slots: the top vertex and the bottom-right and bottom-left vertices. The other peripheral positions are not shown. Their outward position balances equal clearance from the center icon and the two neighboring hex edges.
...
- The hearth is a special center piece and does not consume one of the six peripheral slots available on a hearth hex. Extending that center-piece exception to future buildings is a likely direction, but remains open until buildings are implemented.
